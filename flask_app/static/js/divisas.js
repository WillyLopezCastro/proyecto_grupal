...
myHeaders.append("apikey", "URWp0X9FNZoOLh0lJAXuL2EvJpn0x1W7");
...
async function getHistoricalRates(event, timePeriod) {
  ...
  await fetch(
    `https://api.apilayer.com/exchangerates_data/timeseries?start_date=${timePeriod}&end_date=${today}&base=${baseCurrency}&symbols=${exchangeCurrency}`,requestOptions
  )
...
async function getCurrencies(url,ropt) {
fetch(url,ropt)
    .then(result => result.json())
...
function fetchConversion(values) {
  ...
  fetch(
    `https://api.apilayer.com/exchangerates_data/${exchangeDate}?symbols=${exchangeCurrency}&base=${baseCurrency}`,requestOptions
  )
    .then(result => result.json())
    .then(result => result.rates[exchangeCurrency])
...
getCurrencies("https://api.apilayer.com/exchangerates_data/latest?symbols=&base=USD",requestOptions);
